...
/*   Updated: 2024/05/30 14:30:04 by alaalalm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void distance(t_data *data, t_ray *ray)
{
	if (ray->foundHorzWallHit)
		ray->horzHitDistance = distance_between_points(data->player->posX, data->player->posY, ray->horzWallHitX, ray->horzWallHitY);
	else
		ray->horzHitDistance = INT_MAX;

	if (ray->foundVertWallHit)
		ray->vertHitDistance = distance_between_points(data->player->posX, data->player->posY, ray->vertWallHitX, ray->vertWallHitY);
	else
...
void castRay(t_data *data, double rayAngle)
{
	...
	distance(data, ray);
	line(data, data->player->posX, data->player->posY, ray->wallHitX, ray->wallHitY);
}